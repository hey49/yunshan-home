import React, { useState } from 'react';
import { useIntl, setLocale } from 'umi';
import Grid from '@material-ui/core/Grid';
import Container from '@material-ui/core/Container';
import { Card as MCard } from '@material-ui/core';
import { Slider, Card } from 'antd';
import './index.less';
import r2017ac from './asset/reports/r2017ac.jpeg';
import r2018se2 from './asset/reports/r2018se2.jpg';
import r2018se4 from './asset/reports/r2018se4.jpg';
import r2018ac from './asset/reports/r2018ac.jpeg';
import r2019se1 from './asset/reports/r2019se1.jpg';
import r2019se2 from './asset/reports/r2019se2.jpg';
import r2019se3 from './asset/reports/r2019se3.jpg';
import r2019se4 from './asset/reports/r2019se4.jpg';
import r2019ac from './asset/reports/r2019ac.jpeg';
import r2020se1 from './asset/reports/r2020se1.png';
import r2017a from './asset/reports/r2017a.pdf';
import r2018a from './asset/reports/r2018a.pdf';
import r2019a from './asset/reports/r2019a.pdf';

interface ReportProps {
  initCurr: number
}

const Report: React.FC<ReportProps> = (props) => {
  const intl = useIntl();

  const data = [
    {
      isAnnual: true,
      name: '2017年',
      src: r2017ac,
      href: r2017a,
      down: '云山保护2017年年报.pdf',
      category:['卷首语', '2017大事记', '物种研究', '保护行动', '链接公众', '媒体传播', '机构与成长', '财务报告', '致谢']
    },
    {
      isAnnual: false,
      name: '2018年-第二季度',
      src: r2018se2,
      href: r2018se2,
      down: '云山保护2018第二季度季报.jpg',
      category:['长臂猿种群保护与管理国际研讨会', '科研检测', '保山高黎贡白眉长臂猿保护实施方案', '六一宣传活动', '时间线记事']
    },
    {
      isAnnual: false,
      name: '2018年-第四季度',
      src: r2018se4,
      href: r2018se4,
      down: '云山保护2018第四季度季报.jpg',
      category:['劲草生物多样性嘉年华大理大学站', '板厂基地第一任站长任职', '长臂猿日主题科普游园活动', '腾冲长臂猿种群大调查', '“认识家乡的长臂猿"科普进校园活动', '云山记事']
    },
    {
      isAnnual: true,
      name: '2018年',
      src: r2018ac,
      href: r2018a,
      down: '云山保护2018年年报.pdf',
      category:['卷首语', '2018年云山保护大事记', '保护和科研行动', '板厂保护与科研基地', '保护科普宣传', '传播与筹款', '机构与成长', '财务披露', '致谢']
    },
    {
      isAnnual: false,
      name: '2019年-第一季度',
      src: r2019se1,
      href: r2019se1,
      down: '云山保护2019第一季度季报.jpg',
      category:['云端护猿基地三方共建共管协议签订', '保山昌宁县绿孔雀物种考察', '西双版纳野象谷和易武保护区考察', '“认识家乡的长臂猿"宣传活动', '遗传多样性研究外业工作', '云山记事']
    },
    {
      isAnnual: false,
      name: '2019年-第二季度',
      src: r2019se2,
      href: r2019se2,
      down: '云山保护2019第二季度季报.jpg',
      category:['云端护猿基地', '哀牢山、无量山科普进校园活动', '盈江长臂猿群体调研', '西黑冠长臂猿小使者选拔与培训', '云山保护成立四周年啦', '筹款活动', '云山记事']
    },
    {
      isAnnual: false,
      name: '2019年-第三季度',
      src: r2019se3,
      href: r2019se3,
      down: '云山保护2019第三季度季报.jpg',
      category:['云端护猿基地', '长臂猿栖息地野外工作与社区工作', '自然科考营期', '2019年国际长臂猿日系列活动', '宣传保护工作', '筹款、交流与收获', '暑期实习生、研修生和志愿者的精彩付出', '云山记事']
    },
    {
      isAnnual: false,
      name: '2019年-第四季度',
      src: r2019se4,
      href: r2019se4,
      down: '云山保护2019第四季度季报.jpg',
      category:['板厂片区猿口大调查', '长臂猿栖息地社区调查', '盈江苏典长臂猿栖息地调查', '2019年国际长臂猿日系列活动', '空缺调查', '云山记事']
    },
    {
      isAnnual: true,
      name: '2019年',
      src: r2019ac,
      href: r2019a,
      down: '云山保护2019年年报.pdf',
      category:['卷首语', '云山的2020', '我们的保护工作', '传播与筹款工作', '财务披露', '2019大事记', '致谢']
    },
    {
      isAnnual: false,
      name: '2020年-第一季度',
      src: r2020se1,
      href: r2020se1,
      down: '云山保护2020第一季度季报.png',
      category:['云端护猿基地', '云山未来讨论会', '长臂猿栖息地社区调查', '空缺调查', '2019年报出炉', '云山记事']
    }
  ]


  const [curr, setCurr] = useState<number>(props.initCurr ? props.initCurr : 0);

  const passedStyle = {
    color: '#8da745'
  };


  const masks = {}

  data.forEach((item, index) => {
    masks[index] = {
      style: curr >= index ? passedStyle : null,
      label: item.isAnnual ? <strong style={{fontSize: '24px'}}>{item.name}</strong> : <div>{item.name}</div>,
    }
  })

  return (
    <div className='report-container'>
      <Grid container xs={10} direction='row' justify='space-around' alignItems='center' style={{background: '#ffffff', margin: 'auto', height: '100%'}}>
        <Grid item xs={2} className='report-box'>
          <div style={{height: '70vh'}}>
            <Slider tooltipVisible={false} marks={masks} defaultValue={curr} vertical onChange={(v: any) => setCurr(v)} reverse step={1} min={0} max={data.length - 1}/>
          </div>
        </Grid>
        <Grid item xs={5} className='report-box' container direction='column' justify="flex-start" alignItems="center">
          <Grid item style={{margin: 'auto'}}>
            <a href={data[curr].href} download={data[curr].down}>
              <div className='report-card' >
                <img style={{width: '45vh'}} src={data[curr].src}></img>
              </div>
            </a>
          </Grid>
        </Grid>
        <Grid item xs={2} className='report-box' container direction='column' justify="center" alignItems="flex-start">
          <Grid>
            <div className='report-category'>
              <ul style={{marginTop: '2px'}}>
                {data[curr].category.map(item => <li>{item}</li>)}
              </ul>
            </div>
            <div className='report-tip'>
              {'点击封面下载'}
            </div>
          </Grid>
        </Grid>
      </Grid>
    </div>
  );
}

export default Report;
